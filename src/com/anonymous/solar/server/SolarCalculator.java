...
public class SolarCalculator {
	...
	public SolarResult calculateAllResults(SolarSetup solarSetup, int year) {
		...
		for (int i = 0; i < year; i++) {
			...
			for (int j = 0; j < 12; j++) {
				...
				double daylightHoursInMonth = daysInCurrentMonth * calculateSunlightHours(
						locationInformation.getLatitude(), getDayInMonth(currentMonth));
				...
				if (monthlySystemResults.size() > 0) {
					previousMonthDetails = monthlySystemResults.get(
							monthlySystemResults.size() - 1);
				}
				currentMonthDetails = calculateHardwareState(currentMonthDetails, 
						previousMonthDetails, i, j, solarSetup);
				
				//Calculate power output
				double powerOutput = calculatePowerOutput(currentMonthDetails, solarSetup,
						locationInformation);
				currentMonthDetails.setPowerGenerated(powerOutput);
				
				//Calculate savings
				double income = calculateIncome(currentMonthDetails, newTariff11Fee, 
						newFeedInFee, customerData, solarSetup);
				...
				ROI = calculateROI(monthlySystemResults, currentMonthDetails,
						systemCost);
				}
	...
	private double calculateSunlightHours(double lat, int dayOfYear) {
		...
		double p = Math.asin(0.3975 * Math.cos(0.2163108 + 2 * Math.atan(0.9671396 *
				Math.tan(0.00860 * (dayOfYear-186)))));
	...
	private double calculateSystemCost(SolarSetup solarSystem) {
		...
		for (int i = 0; i < solarPanels.size(); i++) {
			...
			systemCost = systemCost + (currentSolarBank.getPanelCount() * 
	 ...
	 * 
	 * @param month the month in which to find the number of days for
	 ...
	 * 
	 * @param month - the month you want to find a day located in
	...
	/**
	 * This method calculates the current hardware efficency and optimal power output within
	 * the given year and month.
	 * 
	 * @param currentDetails - system breakdown for the current month you're looking at
	 * @param previousMonthDetails - system breakdown for the month previously
	 * @param currentYear - the year in which to find the hardware state for
	 * @param currentMonth - the month in which to find the hardware state for
	 * @param solarSetup - the solar setup in which to calculate the hardware state for
	 * @return - returns a ResultsDetails object that is an updated object of currentDetails
	 * 			 that includes the hardware state of the setup for the month
	 */
	private ResultsDetails calculateHardwareState(ResultsDetails currentDetails, 
			ResultsDetails previousMonthDetails, int currentYear, int currentMonth,
		...
		if (currentMonth == 0 && currentYear == 0) {
			newResultDetails.setInverterEfficiency(
					solarSetup.getInverter().getInverterEfficiency());
			newResultDetails.setInverterOutput(
			...
			//Check if inverter is past it's lifetime or it's efficency is now below 0, when
			//we're at the start of the month for the year
			if (currentInverterEfficency <= 0 || ((currentYear % 
					solarSetup.getInverter().getInverterLifeYears() == 0) && currentMonth == 0)) {
				currentInverterEfficency = solarSetup.getInverter().getInverterEfficiency();
				newResultDetails.setInverterReplaced(true);
			}
			double currentInverterOutput = solarSetup.getInverter().getInverterWattage() *
		...
		for (int k = 0; k < solarSetup.getSolarPanels().size(); k++) {
			...
			if (currentMonth == 0 && currentYear == 0) {
				...
				double panelBankEfficency = 
						...
						(previousMonthDetails.getSolarBanksEfficencyList().get(k) * 
						(currentSolarBank.getPanelType().getPanelLossYear() / 12));
				
				//If the efficency is less is 0 or less, or panel is past its life years
				//it will need replaced and the efficency set back to 100%
				if (panelBankEfficency <= 0 || ((currentYear % 
	...
	/**
	 * This method calculates the power output for the current month details given
	 * 
	 * @param currentMonthDetails - the month to calculate the power output for
	 * @param solarSetup - the setup to calculate the power output for
	 * @param locationData - the location data of where the current setup will be located
	 * @return - a double that represents the power that will be produced by the system
	 * 			 in the given month. (KW/H)
	 */
	private double calculatePowerOutput(ResultsDetails currentMonthDetails, 
			SolarSetup solarSetup, LocationData locationData) {
		double powerOutput = 0.0;
		...
		ArrayList<Double> currentSolarBanksOutputList = 
				currentMonthDetails.getSolarBanksOutputList();
		ArrayList<SolarPanels> solarBanksList = solarSetup.getSolarPanels();
		//Calculate power output of each bank with direction of panels into account as
		...
		for (int i = 0; i < currentSolarBanksOutputList.size(); i++) {
			...
			if (currentBank.getPanelDirection() <= 45 || currentBank.getPanelDirection() > 315) {
				powerOutput = powerOutput + (currentSolarBanksOutputList.get(i) 
						* northDirectionEfficiency);
			}
			//East
			else if (currentBank.getPanelDirection() > 45 && 
					currentBank.getPanelDirection() <= 135) {
				powerOutput = powerOutput + (currentSolarBanksOutputList.get(i) * 
						eastDirectionEfficiency);
			}
			//South
			else if (currentBank.getPanelDirection() > 135 && 
					currentBank.getPanelDirection() <= 225) {
				powerOutput = powerOutput + (currentSolarBanksOutputList.get(i) * 
						southDirectionEfficiency);
			}
			//West
			else {
				powerOutput = powerOutput + (currentSolarBanksOutputList.get(i) * 
						westDirectionEfficiency);
			}
		}
		
		//Check if it's winter in current location, as panels will be less effective at the
		//time
		if (winterMonths.contains(currentMonthDetails.getMonthID())) {
			powerOutput = powerOutput * winterEffciency;
		}
		
		//If the solarPanels are producing more power than the inverter can take then
		...
		//Check for number of cloudy days and take off those day of power output to apply
		//efficency loss
		double numCloudyDays = locationData.getLocationWeatherData().get(
				currentMonthDetails.getMonthID());
		double averageDaylightHoursPerDay = currentMonthDetails.getDaylightHours() / daysInMonth(
				currentMonthDetails.getMonthID());
		
		//Now calculate how much power is produced through the daylight available
		powerOutput = powerOutput * (averageDaylightHoursPerDay * 
	 ...
	 * 
	 * @param currentMonthDetails - the month and setup to calculate the income for
	 * @param tariff11Fee - the current tariff 11 fee for the setup
	 * @param feedInFee - the current feed in fee for the setup
	 * @param customerData - an object representing information about customer usage
	 * @param solarSetup - the setup that is being used
	 * @return a double that represents the income of the given setup for the given month ($)
	 */
	private double calculateIncome(ResultsDetails currentMonthDetails, double tariff11Fee,
			double feedInFee, CustomerData customerData, SolarSetup solarSetup) {
		double income = 0.0;
		double additionalCosts = 0.0;
		
		double replacementGeneration = customerData.getMonthlyAverageUsage() * 
				currentMonthDetails.getDaylightHours();
		double exportedGeneration = currentMonthDetails.getPowerGenerated() - 
	 ...
	 * 
	 * @param resultDetailsList - a list of the results for all previous months of the system
	 * @param currentMonthDetails - the results for the current month
	 * @param initialSystemCost - the cost of building the system
	 * @return - a double that represents the return on investment ($)
	 */
	private double calculateROI(ArrayList<ResultsDetails> resultDetailsList, 
			ResultsDetails currentMonthDetails, double initialSystemCost) {
		double ROI = 0.0;
